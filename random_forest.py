...
class RandomForest(object):
    ...
    # tree number - i..e current tree number being generated - This approach will not work if threads are used
    tree_number = 0
    ...
    def _bootstrapping(self, XX, n):
        ...
        ele_to_get = int(2*n/3)


        if(self.tree_number < 3):
            rec_case = self.tree_number % 3

            if(rec_case == 0):
                samples_xx = XX[0:int(math.floor(2*n/3))]
            elif(rec_case == 1):
                samples_xx = XX[int(math.floor(n/3)):n]
            elif(rec_case == 2):
                samples_xx = XX[0:int(math.floor(n/3))] +  XX[int(math.floor(2*n/3)):n]
        else:
            samples_xx = random.sample(XX,ele_to_get)

        self.tree_number += 1

        rem_ele_count = n-len(samples_xx)

        for i in range(rem_ele_count):
            samples_xx.append(samples_xx[random.randint(0,len(samples_xx)-1)])

        for x_line in samples_xx:
            samples.append(x_line[:-1])
            labels.append(x_line[-1])
    ...
    def fitting(self):
        ...
        for i in range(len(self.bootstraps_datasets)):
            ...
            self.decision_trees[i].node_list['X'] = dataset
            self.decision_trees[i].node_list['y'] = self.bootstraps_labels[i]
            self.decision_trees[i].learn(dataset, self.bootstraps_labels[i])
            #self.decision_trees[i].printPreOrder()
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                print("This case will not occur. Because I have ensured that there is at least one tree which predicts for any dataset")
                pass
            else:
