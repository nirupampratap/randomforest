...
def entropy(class_y):
    ...
    if(len(class_y) <= 0):
        ...
        p_zero = count_y[0] / len(class_y)
        p_one = count_y[1] / len(class_y)

        if(p_zero != 0 and p_one != 0):
            entropy = -1 * p_zero * math.log(p_zero,2) - p_one * math.log(p_one,2)
        else:
            entropy = 0
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if(len(X) > 0 and type(X[0][split_attribute]) is int):
        is_categorical = False

    for index, X_item in enumerate(X):
        if(is_categorical == True):
            if(X_item[split_attribute] == split_val):
                X_left.append(X_item)
                y_left.append(y[index])
            else:
                X_right.append(X_item)
...
def information_gain(previous_y, current_y):
    ...
    pl = float(len(current_y[0])) / len(previous_y)
    pr = float(len(current_y[1])) / len(previous_y)

    info_gain = h - (pl * entropy(current_y[0]) + pr * entropy(current_y[1]))
